...
const steps = ['Approve Loan'];

export default function ApproveLoanStepper(props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await bankContract.methods.approveLoan(props.loanApplication.id)
                ...
                : isNextLoading
                  ...
                  : txReceipt && <>
                    <Typography sx={{ mt: 2, mb: 1, alignSelf: 'center' }} >
                      Approve Loan Request Submitted
                    </Typography>
        ...
        <>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2, mb: '0' }}>
            ...
            <Button onClick={handleNext}
              disabled={isNextLoading || (activeStep === 0 && props.loanApplication.status !== 'APPLIED')}>
